...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    // Only hide cursor after component is fully mounted
    ...
    const tickerFn = () => {
      if (!targetCornerPositionsRef.current || !cursorRef.current || !cornersRef.current || !isMountedRef.current) {
        return;
      }
    ...
    const scrollHandler = () => {
      if (!activeTarget || !cursorRef.current || !isMountedRef.current) return;
    ...
    const mouseDownHandler = () => {
      if (!dotRef.current || !isMountedRef.current) return;
    ...
    const enterHandler = (e: MouseEvent) => {
      ...
      const leaveHandler = () => {
        ...
        resumeTimeout = setTimeout(() => {
          if (!activeTarget && cursorRef.current && spinTl.current && isMountedRef.current) {
